...
def xyz2uv(xyz):
    ...
    '''        # 验证可训练参数 (应该只包含 "lora_" 相关的参数)
        # print("\n可训练的参数:")
        # for name, param in omnigen.named_parameters():
        #     if param.requires_grad:
        #         print(name)
        # AttnProcsLayers 从 LoRAAttnProcessor 模块本身收集参数。
        # 这些处理器现在存储在 omnigen.attn_processors.values() 中。
        # 修改返回语句以收集所有被 PEFT 标记为可训练的参数
    z = c * np.cos(u)
