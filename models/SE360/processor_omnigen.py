...
class OmniGenMultiModalProcessor:
    ...
    def process_image(self, image):
        if isinstance(image, str):
            ...
            # Ensure PIL image objects are also converted to RGB format
            image = image.convert("RGB")
        return self.image_transform(image)

    def process_multi_modal_prompt(self, text, input_images):
        img_num = len(input_images) if input_images is not None else 0
        text = self.add_prefix_instruction(text, img_num)
    ...
    def add_prefix_instruction(self, prompt, img_num=1):
        if prompt == '' and img_num == 1:
            prompt='<img><|image_1|></img>'
        elif prompt == '' and img_num > 1:
            prompt='<img><|image_1|></img>'
            for i in range(2, img_num + 1):
                prompt += f'<img><|image_{i}|></img>'
        user_prompt = "<|user|>\n"
    ...
    def __call__(
        ...
        for i in range(len(instructions)):
            ...
            if cur_input_images is not None:
        ...
        return self.collator(input_data, mode)
