...
import external.py360convert as py360convert
import zipfile
...
def process_file_type(
    ...
    srcdir = os.path.join(base_dir, scan_id,  name)
    ...
    if not is_skyBox:
        paramdict = parse_camera_params(os.path.join(base_dir, scan_id,  "undistorted_camera_parameters", scan_id + ".conf"))
...
if __name__ == "__main__":
    ...
    if not(args.scan_id==None):
        ...
        # Get all entries under m3d_path
        all_entries = os.listdir(args.m3d_path)
        # Filter out the folders
        scan_dirs = [entry for entry in all_entries if os.path.isdir(os.path.join(args.m3d_path, entry))]
        # Create tqdm object using filtered folder list
        scan_id_list = tqdm.tqdm(scan_dirs, desc="Dataset Progress")
